<script lang="ts">
	...
	if (fillDirection == null) fillDirection = "l2r";
	$effect(()=>{
		if (actWidth > 0) {
		...
		if (pathFn) path = pathFn(100, vbHeight);
...
<svg
	class="progressbar progressbar-{style} {cls}"
...
>
	<defs>
		<linearGradient
			id={grId}
			x1="{gradientStartPercX}%"
			x2="{gradientEndPercX}%"
			y1="{gradientStartPercY}%"
			y2="{gradientEndPercY}%"
		>
			{#each $store.series as serie, seriesIdx}
				{#if serie.perc > 0}
					<stop
						offset="{Math.round(
							$store.series[seriesIdx].prevOffset,
						)}%"
						stop-color={$store.series[seriesIdx].color}
					/>
					<stop
						offset="{Math.round(
							$store.series[seriesIdx].prevOffset +
								$store.series[seriesIdx].perc,
						)}%"
						stop-color={$store.series[seriesIdx].color}
					/>
				{/if}
		...
		{#if showProgressValue}
			<mask id={maskId} x="0" y="0" width="100%" {height}>
				<rect
					width="{maskWidth}%"
					height="{maskHeight}%"
					x="{maskX}%"
	...
	{/if}
	<svg
		width="{canvasPercWidth}%"
		height={canvasPercHeight}
		x={canvasPercX}
		y={canvasPercY}
		viewBox="{canvasPercX} {canvasPercY} {canvasPercWidth} {canvasPercHeight}"
	>
